...
router.put('/profile/:id', verifyJWT, verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    if (password && password.trim() && currentPassword && currentPassword.trim()) {
      ...
      console.log('Password verification - Current:', currentPassword, 'Stored:', storedPassword);
      let passwordMatch;
...
router.get('/airdrops', verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT ac.id, ac.collection_id, c.name AS collection_name, ac.airdrop_type,
         ac.token_address, ac.token_symbol, ac.token_decimals, ac.amount_per_nft,
         ac.minimum_threshold, ac.trait_type, ac.trait_value, ac.status,
         ac.activated_at, ac.expires_at, ac.created_at, ac.updated_at,
  ...
  try {
    ...
    if (airdrop_type === 'threshold') {
      const r = await pool.query(
        `SELECT SUM(staked_count * $2) AS max_cost FROM (SELECT wallet_address, COUNT(*) AS staked_count FROM staked_nfts WHERE collection_id = $1 GROUP BY wallet_address HAVING COUNT(*) >= $3) eligible`,
        [collection_id, amount_per_nft, minimum_threshold]
      );
      maxCost = parseFloat(r.rows[0]?.max_cost ?? 0);
    } else {
      const r = await pool.query(
        `SELECT SUM(matching_count * $2) AS max_cost FROM (SELECT wallet_address, COUNT(*) AS matching_count FROM staked_nfts WHERE collection_id = $1 AND traits::jsonb @> $3::jsonb GROUP BY wallet_address HAVING COUNT(*) > 0) eligible`,
    ...
    if (rewardsWallet) {
      try {
        ...
        currentBalance = accounts.reduce((sum, acct) => sum + (acct?.account?.data?.parsed?.info?.tokenAmount?.uiAmount ?? 0), 0);
      } catch (err) {
    ...
    const insertResult = await pool.query(
      `INSERT INTO airdrop_configs (collection_id, airdrop_type, token_address, token_symbol, token_decimals, amount_per_nft, minimum_threshold, trait_type, trait_value, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'inactive') RETURNING *`,
      [collection_id, airdrop_type, token_address, token_symbol, token_decimals !== undefined ? token_decimals : 9, amount_per_nft,
       airdrop_type === 'threshold' ? minimum_threshold : null, airdrop_type === 'trait' ? trait_type : null, airdrop_type === 'trait' ? trait_value : null]
    );
...
router.put('/airdrops/:id', verifyJWT, verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    if (airdrop_type !== undefined && !['threshold', 'trait'].includes(airdrop_type)) return res.status(400).json({ success: false, message: 'airdrop_type must be "threshold" or "trait"' });
    if (updatedType === 'threshold' && (!updatedThreshold || Number(updatedThreshold) <= 0)) return res.status(400).json({ success: false, message: 'threshold type requires minimum_threshold > 0' });
    if (updatedType === 'trait' && (!updatedTraitType || !updatedTraitValue)) return res.status(400).json({ success: false, message: 'trait type requires both trait_type and trait_value' });

    const updates = [];
    const values = [];
    let paramIndex = 1;
    if (token_address !== undefined) { updates.push(`token_address = ${paramIndex++}`); values.push(token_address); }
    if (token_symbol !== undefined) { updates.push(`token_symbol = ${paramIndex++}`); values.push(token_symbol); }
    if (token_decimals !== undefined) { updates.push(`token_decimals = ${paramIndex++}`); values.push(token_decimals); }
    if (amount_per_nft !== undefined) { updates.push(`amount_per_nft = ${paramIndex++}`); values.push(amount_per_nft); }
    if (airdrop_type !== undefined) { updates.push(`airdrop_type = ${paramIndex++}`); values.push(airdrop_type); }
    if (collection_id !== undefined) { updates.push(`collection_id = ${paramIndex++}`); values.push(collection_id); }
    updates.push(`minimum_threshold = ${paramIndex++}`); values.push(updatedType === 'threshold' ? updatedThreshold : null);
    updates.push(`trait_type = ${paramIndex++}`); values.push(updatedType === 'trait' ? updatedTraitType : null);
    updates.push(`trait_value = ${paramIndex++}`); values.push(updatedType === 'trait' ? updatedTraitValue : null);
    updates.push(`updated_at = NOW()`);
    values.push(id);

    const updateResult = await pool.query(`UPDATE airdrop_configs SET ${updates.join(', ')} WHERE id = ${paramIndex} RETURNING *`, values);
...
router.post('/airdrops/:id/deactivate', verifyJWT, verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    if (existingResult.rows.length === 0) return res.status(404).json({ success: false, message: 'Airdrop config not found' });
    const updateResult = await pool.query(`UPDATE airdrop_configs SET status = 'inactive', updated_at = NOW() WHERE id = $1 RETURNING *`, [id]);
    return res.json({ success: true, data: updateResult.rows[0] });
  } catch (error) {
  ...
  try {
    ...
    if (config.status === 'active') {
      const snapshotResult = await pool.query(
        `SELECT wallet_address, eligible_nft_count, token_amount, claimed FROM airdrop_snapshots WHERE airdrop_config_id = $1 ORDER BY token_amount DESC`,
        [id]
      );
    ...
    if (config.airdrop_type === 'threshold') {
      const r = await pool.query(
        `SELECT wallet_address, COUNT(*) AS eligible_nft_count FROM staked_nfts WHERE collection_id = $1 GROUP BY wallet_address HAVING COUNT(*) >= $2`,
        [config.collection_id, config.minimum_threshold]
      );
      wallets = r.rows.map(row => ({ wallet_address: row.wallet_address, eligible_nft_count: parseInt(row.eligible_nft_count), token_amount: parseInt(row.eligible_nft_count) * parseFloat(config.amount_per_nft), claimed: false }));
    } else {
      const r = await pool.query(
        `SELECT wallet_address, COUNT(*) AS eligible_nft_count FROM staked_nfts WHERE collection_id = $1 AND traits::jsonb @> $2::jsonb GROUP BY wallet_address HAVING COUNT(*) > 0`,
...
router.get('/analytics/claims', verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    const params = [];
    let paramIndex = 1;
    if (start_date) { conditions.push(`t.created_at >= ${paramIndex++}`); params.push(start_date); }
    if (end_date) { conditions.push(`t.created_at <= ${paramIndex++}`); params.push(end_date); }
    if (collection_id) { conditions.push(`t.collection_id = ${paramIndex++}`); params.push(collection_id); }
    if (wallet_address) { conditions.push(`t.wallet_address ILIKE ${paramIndex++}`); params.push(wallet_address); }
    const whereClause = `WHERE ${conditions.join(' AND ')}`;
    const baseSelect = `FROM transactions t LEFT JOIN collections c ON t.collection_id = c.id LEFT JOIN collection_rewards cr ON cr.collection_id = t.collection_id AND cr.token_address = t.token_address ${whereClause}`;
    const statsResult = await pool.query(`SELECT COUNT(*) AS count, COALESCE(SUM(t.amount), 0) AS total_distributed, COUNT(DISTINCT t.wallet_address) AS unique_wallets ${baseSelect}`, params);
    const stats = { count: parseInt(statsResult.rows[0].count), total_distributed: parseFloat(statsResult.rows[0].total_distributed), unique_wallets: parseInt(statsResult.rows[0].unique_wallets) };
    if (exportFormat === 'csv') {
      const csvResult = await pool.query(`SELECT t.wallet_address, COALESCE(c.name, '') AS collection_name, COALESCE(cr.token_symbol, '') AS token_symbol, t.amount, t.created_at AS timestamp, COALESCE(t.transaction_hash, '') AS transaction_hash ${baseSelect} ORDER BY t.created_at DESC`, params);
      const csv = ['wallet_address,collection_name,token_symbol,amount,timestamp,transaction_hash', ...csvResult.rows.map(row => [row.wallet_address, row.collection_name, row.token_symbol, row.amount, row.timestamp, row.transaction_hash].map(v => `"${String(v ?? '').replace(/"/g, '""')}"`).join(','))].join('\n');
      res.setHeader('Content-Type', 'text/csv');
      res.setHeader('Content-Disposition', 'attachment; filename="claims-export.csv"');
      return res.send(csv);
    }
    const recordsResult = await pool.query(`SELECT t.id, t.wallet_address, COALESCE(c.name, '') AS collection_name, COALESCE(cr.token_symbol, '') AS token_symbol, t.amount, t.created_at AS timestamp, COALESCE(t.transaction_hash, '') AS transaction_hash, t.status ${baseSelect} ORDER BY t.created_at DESC LIMIT ${paramIndex} OFFSET ${paramIndex + 1}`, [...params, pageLimit, offset]);
...
router.get('/analytics/airdrop-claims', verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    const params = [];
    let paramIndex = 1;
    if (start_date) { conditions.push(`snap.claimed_at >= ${paramIndex++}`); params.push(start_date); }
    if (end_date) { conditions.push(`snap.claimed_at <= ${paramIndex++}`); params.push(end_date); }
    if (collection_id) { conditions.push(`ac.collection_id = ${paramIndex++}`); params.push(collection_id); }
    if (wallet_address) { conditions.push(`snap.wallet_address ILIKE ${paramIndex++}`); params.push(wallet_address); }
    if (airdrop_config_id) { conditions.push(`snap.airdrop_config_id = ${paramIndex++}`); params.push(airdrop_config_id); }
    const whereClause = `WHERE ${conditions.join(' AND ')}`;
    const baseFrom = `FROM airdrop_snapshots snap JOIN airdrop_configs ac ON snap.airdrop_config_id = ac.id JOIN collections c ON ac.collection_id = c.id ${whereClause}`;
    const statsResult = await pool.query(`SELECT COUNT(*) AS count, COALESCE(SUM(snap.token_amount), 0) AS total_airdropped, COUNT(DISTINCT snap.wallet_address) AS unique_wallets ${baseFrom}`, params);
    const stats = { count: parseInt(statsResult.rows[0].count), total_airdropped: parseFloat(statsResult.rows[0].total_airdropped), unique_wallets: parseInt(statsResult.rows[0].unique_wallets) };
    if (exportFormat === 'csv') {
      const csvResult = await pool.query(`SELECT snap.wallet_address, c.name AS collection_name, ac.token_symbol, snap.token_amount AS amount_claimed, snap.claimed_at AS claim_timestamp, COALESCE(snap.claim_tx_hash, '') AS transaction_hash, ac.activated_at, ac.expires_at ${baseFrom} ORDER BY snap.claimed_at DESC`, params);
      const csv = ['wallet_address,collection_name,token_symbol,amount_claimed,claim_timestamp,transaction_hash,activated_at,expires_at', ...csvResult.rows.map(row => [row.wallet_address, row.collection_name, row.token_symbol, row.amount_claimed, row.claim_timestamp, row.transaction_hash, row.activated_at, row.expires_at].map(v => `"${String(v ?? '').replace(/"/g, '""')}"`).join(','))].join('\n');
      res.setHeader('Content-Type', 'text/csv');
      res.setHeader('Content-Disposition', 'attachment; filename="airdrop-claims-export.csv"');
      return res.send(csv);
    }
    const recordsResult = await pool.query(`SELECT snap.id, snap.wallet_address, c.name AS collection_name, ac.token_symbol, snap.token_amount AS amount_claimed, snap.claimed_at, COALESCE(snap.claim_tx_hash, '') AS transaction_hash, ac.activated_at, ac.expires_at, snap.airdrop_config_id, snap.eligible_nft_count ${baseFrom} ORDER BY snap.claimed_at DESC LIMIT ${paramIndex} OFFSET ${paramIndex + 1}`, [...params, pageLimit, offset]);
    return res.json({ success: true, data: { records: recordsResult.rows, total: stats.count, stats } });
  } catch (error) {
