...
// Requirements: 5.1, 5.2, 5.6, 5.9, 5.10
// Returns unclaimed active airdrop snapshots for a wallet within the claim window
...
// Requirements: 5.4
// Returns { token_amount, claim_fee, fee_recipient } for a specific airdrop snapshot
router.post('/airdrops/quote', verifyJWT, async (req, res) => {
  try {
    ...
    // Fetch snapshot — verify unclaimed and within claim window
    const snapshotResult = await pool.query(
      `SELECT snap.token_amount, ac.expires_at
       FROM airdrop_snapshots snap
       JOIN airdrop_configs ac ON snap.airdrop_config_id = ac.id
       WHERE snap.airdrop_config_id = $1
    ...
    // Fetch claim_fee from settings
    const claimFeeResult = await pool.query(
      'SELECT value FROM settings WHERE key_name = $1',
      ['claim_fee']
    );

    // Fetch fee_recipient (rewards_wallet) from settings
    const feeRecipientResult = await pool.query(
      'SELECT value FROM settings WHERE key_name = $1',
      ['rewards_wallet']
    );

    const claim_fee = parseFloat(claimFeeResult.rows[0]?.value || 0);
    const fee_recipient = feeRecipientResult.rows[0]?.value || null;
...
// Verifies fee payment, executes SPL transfer, marks snapshot as claimed
router.post('/airdrops/claim', verifyJWT, async (req, res) => {
  ...
  if (!wallet_address || !airdrop_config_id || !payment_signature) {
    return res.status(400).json({
      success: false,
      message: 'wallet_address, airdrop_config_id, and payment_signature are required'
    });
  }
  ...
  try {
    ...
    // SELECT FOR UPDATE — lock snapshot row to prevent double-claim
    const snapshotResult = await client.query(
      `SELECT snap.id, snap.claimed, snap.token_amount,
              ac.token_symbol, ac.token_address, ac.token_decimals, ac.expires_at,
              ac.collection_id
       FROM airdrop_snapshots snap
       JOIN airdrop_configs ac ON snap.airdrop_config_id = ac.id
       WHERE snap.airdrop_config_id = $1
    ...
    if (new Date(snap.expires_at) <= new Date()) {
      await client.query('ROLLBACK');
      return res.status(410).json({ success: false, message: 'Claim window has expired' });
    }

    // Fetch rewards wallet settings
    const settingsResult = await client.query(
      `SELECT key_name, value FROM settings WHERE key_name IN ('rewards_wallet', 'rewards_wallet_encrypted_key', 'claim_fee')`
    );
    const settings = {};
    ...
    // Check rewards wallet SPL token balance
    let walletBalance = 0;
    try {
      ...
      walletBalance = accounts.reduce((sum, acct) => {
        const uiAmount = acct?.account?.data?.parsed?.info?.tokenAmount?.uiAmount ?? 0;
        return sum + uiAmount;
      }, 0);
    } catch (balanceErr) {
      console.error('Error checking rewards wallet balance:', balanceErr);
      await client.query('ROLLBACK');
    ...
    if (walletBalance < tokenAmount) {
      await client.query('ROLLBACK');
      return res.status(402).json({ success: false, message: 'Insufficient token balance in rewards wallet' });
    }
    ...
    try {
      ...
      console.log(`✅ [AIRDROP CLAIM] SPL transfer completed: ${signature}`);
    } catch (transferErr) {
    ...
    // On success: record transaction and mark snapshot as claimed
    await client.query(
      `INSERT INTO transactions (wallet_address, collection_id, transaction_type, amount, token_symbol, token_address, status, transaction_hash)
       VALUES ($1, $2, 'AIRDROP_CLAIM', $3, $4, $5, 'CONFIRMED', $6)`,
