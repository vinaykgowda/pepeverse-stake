// frontend/src/components/Admin/AirdropAnalytics.jsx

import React, { useState, useEffect, useCallback } from 'react';
import api from '../../services/api';

const AirdropAnalytics = () => {
  const [claims, setClaims] = useState([]);
  const [stats, setStats] = useState({ count: 0, total_airdropped: 0, unique_wallets: 0 });
  ...
  useEffect(() => {
    ...
    api.admin.getAirdrops()
      .then(res => setAirdrops(res.data.data || res.data || []))
      .catch(() => setAirdrops([]));
  }, []);
  ...
  const fetchClaims = useCallback(async (activeFilters, currentPage) => {
    ...
    try {
      ...
      const res = await api.admin.getAirdropClaimsAnalytics(params);
      const data = res.data;
      ...
      setError(err.response?.data?.message || 'Failed to load airdrop claims analytics');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleClearFilters = () => {
    const empty = { start_date: '', end_date: '', collection_id: '', wallet_address: '', airdrop_config_id: '' };
    setFilters(empty);
    setPage(1);
    setAppliedFilters({});
  };
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = 'airdrop-claims-export.csv';
  ...
  return (
    <div>
      <div className="mb-6 flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Airdrop Claims Analytics</h2>
        <button
      ...
      <div className="bg-white rounded-lg shadow-md p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            ...
            <select
              name="airdrop_config_id"
              value={filters.airdrop_config_id}
              onChange={handleFilterChange}
              className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
            >
              <option value="">All Airdrops</option>
              {airdrops.map(a => (
                <option key={a.id} value={a.id}>{a.name || a.airdrop_name || `Airdrop #${a.id}`}</option>
              ))}
            </select>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow-md p-4">
          <p className="text-sm font-medium text-gray-500">Total Airdrop Claims</p>
          <p className="text-2xl font-bold text-gray-900">{stats.count?.toLocaleString() ?? 0}</p>
        </div>
        <div className="bg-white rounded-lg shadow-md p-4">
          <p className="text-sm font-medium text-gray-500">Total Tokens Airdropped</p>
          <p className="text-2xl font-bold text-gray-900">{Number(stats.total_airdropped ?? 0).toLocaleString()}</p>
        </div>
        <div className="bg-white rounded-lg shadow-md p-4">
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          {claims.length === 0 ? (
            <div className="p-12 text-center text-gray-500">
              No airdrop claims found matching the current filters
            </div>
          ) : (
            <>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Airdrop Name</th>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Claim Timestamp</th>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Claim Window Expiry</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {claims.map((claim, idx) => (
                      <tr key={claim.id || idx}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {Number(claim.amount_claimed ?? claim.amount ?? 0).toLocaleString()}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {formatDate(claim.claimed_at || claim.timestamp || claim.created_at)}
                        </td>
