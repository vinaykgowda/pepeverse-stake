// frontend/src/components/DaoAdmin/DaoAirdropAnalytics.jsx
import React, { useState, useEffect, useCallback } from 'react';

const API_BASE = import.meta.env.VITE_API_URL || '';

const DaoAirdropAnalytics = () => {
  ...
  const fetchClaims = useCallback(async (activeFilters, currentOffset) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/v1/dao-admin/analytics/airdrop-claims?${params}`, {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-indigo-900">DAO Airdrop Analytics</h2>
        <p className="text-sm text-indigo-600 mt-1">DAO_AIRDROP_CLAIM transaction history</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-indigo-500">
          <p className="text-sm font-medium text-indigo-600">Total Airdrop Claims</p>
      ...
      {stats.by_token && stats.by_token.length > 0 && (
        <div className="bg-white rounded-lg shadow-md p-4 mb-6 border border-indigo-100">
          <h3 className="text-sm font-medium text-indigo-700 mb-3">DAO Airdrop Tokens Distributed</h3>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden border border-indigo-100">
          {claims.length === 0 ? (
            <div className="p-12 text-center text-indigo-400">No DAO airdrop claims found</div>
          ) : (
            <>
...
export default DaoAirdropAnalytics;
