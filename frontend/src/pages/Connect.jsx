...
const Connect = () => {
  const { connect, loading, error, connected } = useWallet(); // Add 'connected'
  const [selectedWallet, setSelectedWallet] = useState('');
  const navigate = useNavigate();

  // Navigate when connection is successful
  useEffect(() => {
    if (connected) {
      navigate('/staking');
    }
  ...
  const handleConnect = async (walletName) => {
    ...
    // Navigation will happen automatically via useEffect when connected becomes true
  };
  ...
  return (
    <div className="max-w-md mx-auto my-12 p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="space-y-4">
        <button
          ...
          className={`w-full flex items-center justify-between p-4 border rounded-lg
            ${loading && selectedWallet === 'phantom' ? 'bg-purple-50 border-purple-300' : 'hover:bg-gray-50 border-gray-300'}`}
        >
        ...
        <button
          ...
          disabled={loading}
          className={`w-full flex items-center justify-between p-4 border rounded-lg
            ${loading && selectedWallet === 'solflare' ? 'bg-orange-50 border-orange-300' : 'hover:bg-gray-50 border-gray-300'}`}
        >
          <div className="flex items-center">
      ...
      <div className="mt-8 text-center text-sm text-gray-500">
        <p>
          New to Solana?{' '}
          <a href="https://phantom.app/" target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-500">
            Get a wallet
          </a>
        </p>
      </div>
